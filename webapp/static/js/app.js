...
var BDAY_VIDEO_PROMPT = `Аутентичное вертикальное видео со смартфона, 9:16, 1080x1920, ручная съёмка ночью. Мобильная документалка: тряска, лёгкий смаз, низкосветовой шум и зерно. Макс. фотореализм — как видео от друга, НЕ кино, НЕ стилизация. Свет только от фонарей.
【Длит.】10 сек
...
var TRENDS = {
    "birthday-photo": {
        ...
        desc: { ru: "Загрузите свое фото. Измените цифру в тексте промпта на нужную вам. Вы можете изменить одежду и прическу в тексте промпта на свое усмотрение или оставить как есть.", en: "Upload your photo. Change the number in the prompt text to the one you need. You can change the outfit and hairstyle in the prompt or leave as is.", es: "Sube tu foto. Cambia el número en el texto del prompt al que necesites. Puedes cambiar la ropa y el peinado en el prompt o dejarlo como está." }
    },
    "birthday-video": {
        ...
        ratio: "9:16", minPhotos: 1, maxPhotos: 1, prompt: BDAY_VIDEO_PROMPT,
    ...
    "yacht-video": {
        ...
        ratio: "9:16", quality: "480p", minPhotos: 1, maxPhotos: 7, prompt: YACHT_VIDEO_PROMPT,
...
function showTplDetail(id) {
    ...
    document.getElementById("tpl-gen").addEventListener("click", function() { tplGenerate(tpl, this); });

    showPage("tpl-detail");
}

async function tplGenerate(tpl, btn) {
    var promptEl = document.getElementById("tpl-prompt");
    var prompt = promptEl ? promptEl.value.trim() : tpl.prompt;
